...
async def impedance_check_demo(manager: DeviceManager, device_id: str):
    ...
    try:
        ...
        for channel_id, impedance in impedance_results.items():
            quality = "Good" if impedance < 10000 else "Fair" if impedance < 20000 else "Poor"
...
async def device_discovery_demo(manager: DeviceManager):
    ...
    try:
        ...
        if discovered_devices:
            ...
            logger.info("No devices found. Make sure devices are powered on and in range.")
            ...
            try:
                while True:
                    ...
                    if notification.get('type') == 'impedance_check_complete':
                        ...
                        logger.info(f"  Channels checked: {data.get('channels_checked')}")
                        logger.info(f"  Good channels: {data.get('quality_summary', {}).get('good_channels')}")
    ...
    async with DeviceManager() as manager:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"  Sampling rates: {capabilities.supported_sampling_rates}")
                logger.info(f"  Has impedance check: {capabilities.has_impedance_check}")
                logger.info(f"  Has battery monitor: {capabilities.has_battery_monitor}")
