    ...
    def store_in_bigtable(self, processed_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            row.set_cell(
                'data',
                'raw',
                json.dumps(processed_data['data'].tolist() if isinstance(processed_data['data'], np.ndarray) else processed_data['data']),
    ...
    def send_metrics(self, processing_time: float, success: bool) -> None:
        ...
        try:
            ...
            point = monitoring_v3.Point({
                "interval": interval,
                "value": {"double_value": processing_time}
            })
    ...
    def process(self, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.store_in_bigtable(storage_data):
                success = True
                logger.info(f"Successfully processed {self.signal_type} data for session {message_data['session_id']}")
