...
class DeviceControlMCPServer(BaseNeuralMCPServer):
    ...
    async def _disconnect_device(
        ...
        return await self.handlers.disconnect_device(
            device_id=device_id, force=force, save_session=save_session
        )
    ...
    async def _stop_recording(
        ...
        return await self.handlers.stop_recording(
            device_id=device_id, save_data=save_data, auto_analyze=auto_analyze
        )

    async def _check_impedance(
        self,
        device_id: str,
        channels: Optional[List[int]] = None,
    ...
    async def _monitor_signal_quality(
        self,
        device_id: str,
        duration: float = 10,
        channels: Optional[List[int]] = None,
    ...
    async def _run_device_diagnostics(
        ...
        return await self.handlers.run_device_diagnostics(
            device_id=device_id, test_suite=test_suite, generate_report=generate_report
        )

    async def _calibrate_device(
        self,
        device_id: str,
