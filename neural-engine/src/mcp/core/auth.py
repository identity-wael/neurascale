...
class MCPAuthManager:
        ...
        try:
            ...
            for key_id, key_info in self.api_keys.items():
                if key_info.get("hash") == key_hash:
                    ...
                    if (
                        expires_at
                        and datetime.fromisoformat(expires_at) < datetime.utcnow()
                    ):
                        return None
    ...
    def generate_api_key(
        self, user_id: str, permissions: list = None, expires_days: int = None
    ) -> Dict[str, str]:
    ...
    def generate_jwt_token(
        self, user_id: str, permissions: list = None, expires_hours: int = None
    ) -> str:
    ...
    def _hash_api_key(self, api_key: str) -> str:
        ...
        """
        return hmac.new(
            self.api_key_salt.encode(), api_key.encode(), hashlib.sha256
        ).hexdigest()
