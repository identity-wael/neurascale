...
from ..visualization.omniverse.neural_omniverse_connector import NeuralOmniverseConnector
...
visualization_api = Blueprint("visualization", __name__, url_prefix="/api/v1/visualization")
...
def get_visualization_status() -> Response:
    ...
    try:
        ...
        status = {
            ...
            "streaming_active": hasattr(connector, "_streaming_active") and connector._streaming_active,
...
async def start_streaming() -> Response:
    ...
    try:
        ...
        if source == "live":
            ...
            success = await connector._start_simulated_stream(sample_rate)
        else:
...
async def generate_heatmap() -> Response:
    ...
    try:
        ...
        if heatmap_type == "2d_projection":
            ...
            return jsonify({"error": f"Heatmap type {heatmap_type} not yet implemented"}), 501
        ...
        if hasattr(connector, "vr_controller"):
            ...
            if success:
                return jsonify({
                    "status": "enabled",
                    "platform": platform,
                    "controllers_detected": connector.vr_controller.left_controller is not None
                }), 200
