...
class EncryptionBenchmark:
    ...
    def __init__(self, project_id: str):
        ...
        self.encryption = NeuralDataEncryption(
            project_id=project_id, enable_caching=True
        )
    ...
    def benchmark_batch_operations(self):
        ...
        for batch_size in batch_sizes:
            ...
            for data in batch_data:
                encrypted, _ = self.encryption.encrypt_neural_data(data, encrypted_dek)
    ...
    def generate_report(self):
        ...
        ax = axes[0, 0]
        df.groupby("operation")["throughput_mbps"].mean().plot(kind="bar", ax=ax)
        ...
        for op in df["operation"].unique():
            ...
            ax.scatter(
                op_data["data_size_mb"], op_data["duration_ms"], label=op, alpha=0.7
            )
