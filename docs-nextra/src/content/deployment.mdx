...
NeuraScale uses a three-project GCP architecture with centralized Terraform state:
...
| **State Management** | `neurascale` (Project ID: 555656387124) | Terraform state bucket | `US` |

<Callout type="info">
  **Important**: Terraform state is stored in the `neurascale-terraform-state` bucket in the `neurascale` project. All environments share this state bucket with different prefixes.
</Callout>
