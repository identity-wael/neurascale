...
class NeuralSignalQualityCheck(beam.DoFn):
    ...
    def process(self, element: Dict[str, Any]) -> Iterator[Dict[str, Any]]:
        try:
            ...
            min_val = np.min(data)
            if (
                np.sum(data == max_val) > len(data) * 0.01
                or np.sum(data == min_val) > len(data) * 0.01
            ):
    ...
    def process(self, element: Dict[str, Any]) -> Iterator[Dict[str, Any]]:
        try:
            ...
            freqs, psd = scipy_signal.welch(
                data, self.sampling_rate, nperseg=min(256, len(data))
            )
        ...
        try:
            ...
            stats = {
                ...
                "zero_crossing_rate": float(
                    np.sum(np.diff(np.sign(data)) != 0) / len(data)
                ),
            }
    ...
    def process(self, element: Dict[str, Any]) -> Iterator[Dict[str, Any]]:
        try:
            ...
            spectral_centroid = float(
                np.sum(fft_freq_pos * power_spectrum) / np.sum(power_spectrum)
            )
            ...
            spectral_features = {
                ...
                "spectral_entropy": float(
                    -np.sum(power_spectrum * np.log2(power_spectrum + 1e-10))
                    / np.log2(len(power_spectrum))
                ),
            }
        ...
        try:
            ...
            for lag in autocorr_lags:
                if lag < len(data):
                    ...
                    autocorr_features[f"autocorr_lag_{lag}"] = (
            ...
            def sample_entropy(data: np.ndarray, m: int = 2, r: float = 0.2) -> float:
                ...
                def _maxdist(xi: np.ndarray, xj: np.ndarray, m: int) -> float:
                    return float(
                        max([abs(ua - va) for ua, va in zip(xi[0:m], xj[0:m])])
                    )
            ...
            temporal_features = {
                **autocorr_features,
                "sample_entropy": float(
                    sample_entropy(data[: min(len(data), 100)])
    ...
    def run(self, pubsub_topic: str, streaming: bool = True) -> None:
        ...
        with beam.Pipeline(options=options) as pipeline:
                ...
                neural_data = (
                    ...
                    >> beam.Map(
                        lambda x: window.TimestampedValue(
                            x, x.get("timestamp", datetime.utcnow().timestamp())
                        )
                    )
                    | "Window"
                    >> beam.WindowInto(
                        ...
                        trigger=AfterWatermark(
                            early=AfterProcessingTime(0.01)
                        ),  # 10ms early firing
            ...
            processed_data = (
                ...
                >> beam.ParDo(ExtractStatisticalFeatures())
...
def main() -> None:
    ...
    parser.add_argument(
        "--pubsub-topic", required=True, help="Pub/Sub topic for neural data"
    )
    parser.add_argument(
        "--streaming", action="store_true", help="Run in streaming mode"
    )
